alpine_url=http://dl-cdn.alpinelinux.org/alpine/v3.24

tools_tar=apk-tools-static-3.0.6-r0.apk
tools_url=$alpine_url/main/armv7/$tools_tar

firmware_tar=linux-firmware-other-20260519-r0.apk
firmware_url=$alpine_url/main/armv7/$firmware_tar

linux_dir=tmp/linux-6.18
linux_ver=6.18.38-xilinx
...
for tar in linux-firmware-ath9k_htc-20260519-r0.apk linux-firmware-brcm-20260519-r0.apk linux-firmware-cypress-20260519-r0.apk linux-firmware-rtlwifi-20260519-r0.apk
do
...
zip -r red-pitaya-alpine-3.24-armv7-`date +%Y%m%d`-$project.zip apps boot.bin cache modloop red-pitaya.apkovl.tar.gz start.sh wifi
